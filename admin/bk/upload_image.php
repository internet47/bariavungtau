...
function uploads($name)
{
		...
		$allowtype = array('bmp', 'gif', 'jpg', 'jpe', 'png','flv');        // allowed extensions
		if(isset($_FILES[$name]) && strlen($_FILES[$name]['name']) > 1)
		{
	  		$uploadpath = $uploadpath . basename( $_FILES[$name]['name']);       // gets the file name
	  		$sepext = explode('.', strtolower($_FILES[$name]['name']));
	  		$type = end($sepext);       // gets extension
	  		list($width, $height) = getimagesize($_FILES[$name]['tmp_name']);     // gets image width and height
	  		$err = '';         // to store the errors
	
	  		// Checks if the file has allowed type, size, width and height (for images)
	  		if(!in_array($type, $allowtype)) 
				$err .= 'The file: <b>'. $_FILES[$name]['name']. '</b> not has the allowed extension type.';
	  		if($_FILES[$name]['size'] > $max_size*1000) 
	  		...
	  		if($err == '')
			{
				...
				{ 
					  //echo 'File: <b>'. basename( $_FILES[$name]['name']). '</b> successfully uploaded:';
					  $value = basename( $_FILES[$name]['name']); 
					  return $value;
