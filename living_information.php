...
<script type="text/javascript"> 
	...
	var Mylocation1;
	var Mylocation2;
	var file_xml ="admin/data/xmlv2.xml"; 
	var homexml = "admin/data/home.xml"; 
	...
	$.ajax({url:homexml, cache:false, success: function(data)
	 {
		   ...
		   {
				Mylocation1 = parseFloat($(this).find("lat").text());
				Mylocation2 = parseFloat($(this).find("lng").text());
				
				var myLatlng = new google.maps.LatLng(Mylocation1,Mylocation2); 
				var myOptions = {
				zoom: 16,
				center: myLatlng,
...
function list()
{
		...
		$.ajax({url: file_list, cache:false}).done(function(data)
		{
				$(data).find("List").each(function()
				{
				 ...
				 $("#wrap #navi ul").append(elem);
				});
				
				
				$('.typePosition').change(function(e) 
				{
					if (!this.checked)
						{//alert('uncheck');
						getHome();
