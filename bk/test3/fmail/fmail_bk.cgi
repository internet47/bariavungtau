...
#							}
					...
					if(($text_min > $textsize && $text_min ne $null) || ($textsize > $text_max && $text_max ne $null)){
						$sanitizing_error{$elements_id} = "文字数は${text_min}文字から${text_max}文字の範囲で入力してください。<br />";
						push @session_data,"en${elements_id}_error=" . $sanitizing_error{$elements_id};
						$sanitizing_error_flag = 1;
					}
...
#	$scriptObj = "\n<script type=\"text/javascript\">\n\nvar exampleObj = new Array();\nvar postvalueObj = new Array();\nvar elementsetObj = new Array();\n";## fix / keep selectObj 090723
	...
	if($_GET{'mode'} eq "error"){
		...
		for($cnt=0;$cnt<@elements;$cnt++){
			($elements_id,$num,$name,$type_of_element,$html_size,$html_rows,$html_cols,$html_id,$element_type,$check_type,$on_event,$on_focus,$text_min,$text_max,$enable_filetypes,$filesize_min,$filesize_max,$checked_min,$checked_max,$element_valus,$element_text,$html_example,$note,$element_error_message,$must_icon,$default_value,$system_disp_false) = split(/\t/,$elements[$cnt]);
			$elementname = "en${elements_id}";
	...
	#入力画面での項目出力等
	for($cnt=0;$cnt<@elements;$cnt++){
		($elements_id,$num,$name,$type_of_element,$html_size,$html_rows,$html_cols,$html_id,$element_type,$check_type,$on_event,$html_tag_free,$text_min,$text_max,$enable_filetypes,$filesize_min,$filesize_max,$checked_min,$checked_max,$element_valus,$element_text,$html_example,$note,$element_error_message,$must_icon,$default_value,$system_disp_false,$html_tag_free_top) = split(/\t/,$elements[$cnt]);
		$elementname = "en${elements_id}";
		...
		if($element_type eq "text"){
			...
			$elementtag = "${disp_error_message}${html_tag_free_top}<input type=\"text\" name=\"en${elements_id}\" id=\"en${elements_id}\"${eclass} value=\"${values[0]}\"${html_size}${on_event} />${note}\n";
		}
		...
		if($element_type ne "hidden"){
			...
			if($display_element_flag){
				...
				#talbe定義の場合
					...
					$contents .= "<tr id=\"r_en${elements_id}\">\n";
					...
					$contents .= "<dl class=\"mailform\" id=\"r_en${elements_id}\">\n";
					$contents .= "<dt${classname} id=\"h_en${elements_id}\">${must_icontag}${name}</dt>\n";
...
if($redirect){
	...
	print "Set-Cookie: session=${session}; path=${cookie_path}; expires=\n\n";
}
else {
