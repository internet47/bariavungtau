...
<form id="user_add" action="?m=$form{'m'}&a=save" method="POST" onSubmit="return checkUserEdit(this,'${submit}');">
	...
	<table cellpadding="0" cellspacing="0" class="sheet">
		...
		<tr>
			<th>条件名</th>
			<td><input type="text" name="conditional_name" id="conditional_name" value="$gethash{'conditional_name'}" size="30" /></td>
		</tr>
		<tr>
			<th $mailaddress_error_bg>送信先</th>
			<td $mailaddress_error_bg>
				<span class="caution">$mailaddress_error_msg</span>
				<input type="text" name="mailaddress" id="mailaddress" value="$gethash{'mailaddress'}" size="40" onblur="email_check(this.id)" />
		...
		<tr>
			<th>CC</th>
			<td>
				<input type="text" name="cc" id="cc" value="$gethash{'cc'}" size="40" onblur="email_check(this.id)" /><br />
				フォームから受け取った値を反映 →
		...
		<tr>
			<th>BCC</th>
			<td>
				<input type="text" name="bcc" id="bcc" value="$gethash{'bcc'}" size="40" onblur="email_check(this.id)" /><br />
				フォームから受け取った値を反映 →
		...
		<tr>
			...
			<td $subject_error_bg>
				...
				<input type="text" name="subject" id="subject" value="$gethash{'subject'}" size="40" /><br />
				フォームから受け取った値を反映 →
