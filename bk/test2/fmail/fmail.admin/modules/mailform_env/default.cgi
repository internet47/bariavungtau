...
<form id="user_add" action="?m=$form{'m'}&a=save" method="POST" onSubmit="return checkUserEdit(this,'${submit}');">
	<table cellpadding="0" cellspacing="0" class="sheet">
		...
		<tr>
			<th>対応言語モード</th>
			<td>
				<input type="radio" name="setlang" id="setlang" value="ja"$setlang_ja_checked /> 日本語モード（推奨）　
				<input type="radio" name="setlang" id="setlang" value="utf8"$setlang_utf8_checked /> 多言語モード<br />
				<p>
					<span class="info">多言語モードは、utf8エンコードを使っているため一部の機種依存文字は読めませんが、世界各国の言語に対応可能です。</span>
					<span class="info">多言語モードは、メール受信時フィーチャーフォンでは文字化けします。（utf8を認識できない為）</span>
				</p>
			</td>
		</tr>
		<tr>
